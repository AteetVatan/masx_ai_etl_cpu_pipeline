...
class TestPipelineManager:
    ...
    @pytest.mark.asyncio
    async def test_process_article_success(self, pipeline_manager, sample_article_data, sample_enriched_data):
        ...
        with patch.object(pipeline_manager, '_scrape_article') as mock_scrape:
            with patch.object(pipeline_manager, '_clean_text') as mock_clean:
                with patch.object(pipeline_manager, '_geotag_article') as mock_geotag:
                    with patch.object(pipeline_manager, '_find_images') as mock_find_images:
    ...
    @pytest.mark.asyncio
    async def test_process_article_scraping_error(self, pipeline_manager, sample_article_data):
    ...
    @pytest.mark.asyncio
    async def test_process_article_cleaning_error(self, pipeline_manager, sample_article_data):
    ...
    @pytest.mark.asyncio
    async def test_process_article_geotagging_error(self, pipeline_manager, sample_article_data):
    ...
    @pytest.mark.asyncio
    async def test_process_article_image_search_error(self, pipeline_manager, sample_article_data):
    ...
    async def test_process_batch_success(self, pipeline_manager, sample_batch_data):
        ...
        with patch.object(pipeline_manager, '_fetch_articles_batch') as mock_fetch:
            with patch.object(pipeline_manager, '_update_articles_batch') as mock_update:
                with patch.object(pipeline_manager, 'process_article') as mock_process:
                    ...
                    result = await pipeline_manager.process_batch(["test_article_1", "test_article_2"])
    ...
    @pytest.mark.asyncio
    async def test_process_batch_partial_failure(self, pipeline_manager, sample_batch_data):
    ...
    async def test_scrape_article_primary_success(self, pipeline_manager):
        ...
        with patch('src.pipeline.pipeline_manager.scraper') as mock_scraper:
            mock_scraper.scrape_article.return_value = {"url": "test", "content": "test content"}
    ...
    async def test_scrape_article_fallback_success(self, pipeline_manager):
        ...
        with patch('src.pipeline.pipeline_manager.scraper') as mock_scraper:
            with patch('src.pipeline.pipeline_manager.fallback_scraper') as mock_fallback:
                mock_scraper.scrape_article.side_effect = Exception("Primary failed")
                mock_fallback.scrape_article.return_value = {"url": "test", "content": "test content"}
                
                result = await pipeline_manager._scrape_article("https://example.com")
                
                assert result == {"url": "test", "content": "test content"}
                mock_scraper.scrape_article.assert_called_once_with("https://example.com")
                mock_fallback.scrape_article.assert_called_once_with("https://example.com")
    ...
    async def test_find_images(self, pipeline_manager, sample_article_data):
        ...
        with patch('src.pipeline.pipeline_manager.image_finder') as mock_finder:
            mock_finder.generate_search_queries.return_value = ["test query", "test article"]
            ...
            assert result["image_search_metadata"]["queries_used"] == ["test query", "test article"]
    ...
    async def test_fetch_articles_batch(self, pipeline_manager, sample_batch_data):
        ...
        with patch('src.pipeline.pipeline_manager.db_client') as mock_db:
            ...
            result = await pipeline_manager._fetch_articles_batch(["article1", "article2", "article3"])
    ...
    async def test_update_articles_batch(self, pipeline_manager):
        ...
        with patch('src.pipeline.pipeline_manager.db_client') as mock_db:
            mock_db.client = AsyncMock()
            mock_db.connect.return_value = None
            mock_db.update_articles_batch.return_value = (2, 0)  # 2 successful, 0 failed
    ...
    async def test_get_database_stats(self, pipeline_manager):
        ...
        with patch('src.pipeline.pipeline_manager.db_client') as mock_db:
            mock_db.client = AsyncMock()
            mock_db.connect.return_value = None
            mock_db.get_processing_stats.return_value = {"pending": 10, "completed": 50, "failed": 5}
    ...
    async def test_health_check(self, pipeline_manager):
        ...
        with patch('src.pipeline.pipeline_manager.thread_pool') as mock_thread_pool:
            with patch('src.pipeline.pipeline_manager.db_client') as mock_db:
                with patch('src.pipeline.pipeline_manager.geotagger') as mock_geotagger:
                    with patch('src.pipeline.pipeline_manager.image_finder') as mock_image_finder:
                        # Mock all components
                        ...
                        assert result["components"]["thread_pool"]["status"] == "healthy"
                        assert result["components"]["database"]["status"] == "healthy"
                        assert result["components"]["geotagger"]["status"] == "healthy"
                        assert result["components"]["image_finder"]["status"] == "healthy"
    ...
    async def test_health_check_database_error(self, pipeline_manager):
        ...
        with patch('src.pipeline.pipeline_manager.thread_pool') as mock_thread_pool:
            with patch('src.pipeline.pipeline_manager.db_client') as mock_db:
                ...
                assert "Connection failed" in result["components"]["database"]["details"]
