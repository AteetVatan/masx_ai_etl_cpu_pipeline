...
class DatabaseClientAndPool:
        ...
        if self.pool:
            try:
                async with self.pool.acquire() as conn:
                    ...
                    self.logger.info(f"PostgreSQL pool connection verified: {version[:50]}...")
            except Exception as e:
    ...
    async def fetch_feed_entries(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.info(f"Fetched {len(result.data)} feed entries from {table_name}")
            return result.data
    ...
    async def fetch_feed_entries_by_flashpoint_id(self, flashpoint_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if result.data is None:
                self.logger.warning(f"No feed entries found for flashpoint_id {flashpoint_id} in table {table_name}")
                return []
            
            self.logger.info(f"Fetched {len(result.data)} feed entries for flashpoint_id {flashpoint_id} from {table_name}")
            return result.data
    ...
    async def update_articles_batch(
        ...
        try:
            ...
            self.logger.info(f"Batch update completed: {successful_updates} successful, {failed_updates} failed")
        ...
        try:
            ...
            article_data = {
                ...
                "entities": (
                    ...
                    if enriched_data.entities else None
                ),
                "geo_entities": (
                    [g.model_dump() for g in enriched_data.geo_entities]
                    if enriched_data.geo_entities else None
                ),
            }
            ...
            if result.data:
                self.logger.debug(f"✅ Saved processed article for URL: {enriched_data.url}")
                return True
            else:
                self.logger.warning(f"⚠️ Failed to save processed article for URL: {enriched_data.url}")
                return False
