...
class WorkerStats:
    ...
    def is_idle(self) -> bool:
        ...
        if self.last_activity is None:
            return True
        return (
            time.time() - self.last_activity.timestamp() > 60
...
class DynamicThreadPool:
    ...
    def __init__(self, max_workers: Optional[int] = None):
        ...
        logger.info(
            f"Initialized DynamicThreadPool with max_workers={self.max_workers}"
        )

    def _create_executor(self) -> ThreadPoolExecutor:
        """Create a new ThreadPoolExecutor with current worker count."""
        return ThreadPoolExecutor(
            max_workers=self.current_workers, thread_name_prefix="masx-worker"
        )
    ...
    def _update_worker_stats(
        self, worker_id: str, task_name: str, processing_time: float
    ) -> None:
    ...
    def _should_scale_up(self) -> bool:
        ...
        with self._stats_lock:
            ...
            busy_workers = sum(
                1 for stats in self._worker_stats.values() if not stats.is_idle
            )
    ...
    def _should_scale_down(self) -> bool:
        ...
        with self._stats_lock:
            ...
            idle_workers = sum(
                1 for stats in self._worker_stats.values() if stats.is_idle
            )
    ...
    def _auto_scale(self) -> None:
        ...
        if self._should_scale_up():
            ...
            if self.current_workers != old_count:
                logger.info(
                    f"Auto-scaling UP: {old_count} -> {self.current_workers} workers"
                )
                self._last_scale_time = time.time()
            ...
            if self.current_workers != old_count:
                logger.info(
                    f"Auto-scaling DOWN: {old_count} -> {self.current_workers} workers"
                )
                self._last_scale_time = time.time()
                self._restart_executor()
    ...
    def submit_task(
        self, func: Callable, *args, task_name: str = "unknown", **kwargs
    ) -> Any:
        """
        ...
        def tracked_func(*fargs, **fkwargs):
            ...
            try:
                ...
                self._update_worker_stats(
                    worker_id, f"{task_name} (ERROR)", processing_time
                )
                raise
    ...
    def map_tasks(
        self, func: Callable, iterable: List[Any], task_name: str = "batch"
    ) -> List[Any]:
        """
        ...
        for i, item in enumerate(iterable):
            future = self.submit_task(func, item, task_name=f"{task_name}_{i+1}")
        ...
        with self._stats_lock:
            ...
            return {
                ...
                "performance": {
                    ...
                    "tasks_per_second": (
                        self._total_tasks_completed / uptime if uptime > 0 else 0
                    ),
    ...
    def is_healthy(self) -> bool:
        ...
        with self._stats_lock:
            ...
            for stats in self._worker_stats.values():
                if (
                    stats.last_activity
                    and (current_time - stats.last_activity.timestamp()) < 300
                ):  # 5 minutes
                    return True
