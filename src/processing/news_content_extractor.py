...
from src.scraping import Crawl4AIExtractor, get_beautiful_soap_extractor, get_trafilatura_extractor, get_final_url
...
class NewsContentExtractor:
    ...
    async def _scrape_multilang_feeds(self, url: str, proxies: list[str]) -> ExtractResult:
        """
        ...
        try:
            ...
            self.logger.info(
                f"NewsContentExtractor:Scraping ------ {url}"
            )
            ...
            try:
                ...
                traf_result: ExtractResult = await trafilatura_extractor.scrape_article(url, proxy)
            ...
            try:
                proxy = choice(proxies)
                crawl_result: ExtractResult  = await self.crawl4AIExtractor.crawl4ai_scrape_with_retry(url)
    ...
    def _merge_results(self, traf_result: ExtractResult, crawl_result: ExtractResult) -> ExtractResult:
        """
